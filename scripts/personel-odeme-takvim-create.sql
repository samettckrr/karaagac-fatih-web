...
CREATE TABLE IF NOT EXISTS public.personel_odeme_takvim (
  ...
  tip text NOT NULL CHECK (tip IN ('hediye', 'kira', 'cocuk', 'yakacak', 'ikramiye', 'elbise', 'asker', 'yeni_dogan', 'yol_yardimi', 'dugun', 'sigorta')),
...
COMMENT ON COLUMN public.personel_odeme_takvim.tip IS 'Ödeme tipi: hediye, kira, cocuk, yakacak, ikramiye, elbise, asker, yeni_dogan, yol_yardimi, dugun, sigorta';
COMMENT ON COLUMN public.personel_odeme_takvim.tutar IS 'Ödeme tutarı (numeric)';
COMMENT ON COLUMN public.personel_odeme_takvim.verildigi_tarih IS 'Ödemenin verildiği tarih (date); sigorta tipinde null';
