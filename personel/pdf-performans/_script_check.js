    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      waitForSupabaseAndOrtak(() => {
        ...
        const PLANLANAN_2026_GIDERLER_TIP = (() => {
          ...
          // FARUK NAFİZ ÖZGAN: Hediye 12x27.500=330.000, İkramiye 2x27.500=55.000, Çocuk 12x1.400=16.800, Elbise 10.700, Kira 241.000, Yakacak 18.000
          ...
          // İBRAHİM AY: Hediye 12x33.000=396.000, İkramiye 2x33.000=66.000, Çocuk 12x2.800=33.600, Elbise 10.700, Kira 241.000, Yakacak 18.000, Yol 30.000
          ekle('İBRAHİM AY', {
          ...
          // MAHMUT SOLMAZ: Hediye 12x27.500=330.000, İkramiye 2x27.500=55.000, Çocuk 12x1.400=16.800, Elbise 10.700, Kira 204.000, Yakacak 18.000
          ekle('MAHMUT SOLMAZ', {
            hediye: 330000, // 12 x 27.500
            ikramiye: 55000, // 2 x 27.500
            cocuk: 16800, // 12 x 1.400
        ...
        const PERSONEL_DETAYLARI = (() => {
          ...
          ekle('KEREM KOCAOĞLU', {
            ...
            aileDurumu: 'Evli, 1 çocuğu var (1 erkek)',
            aktifVazifeler: 'Yardımcı Personel, Kıraat Ders Hocası, Dernek Muhasebe Mesulü, Teknik İşler Mesulü, Nehari Yardımcı Personel',
            kategori: 'Hocaefendi'
          });
        ...
        async function yukleVeriler() {
          try {
            ...
            if (odemeYillikAnalizEl) {
              ...
              yillikOdemeHtml += `<tr><td><strong>Toplam</strong></td><td class="sayi"><strong>${fmt(toplamOdeme)}</strong></td></tr>`;
              yillikOdemeHtml += '</tbody></table>';
              odemeYillikAnalizEl.innerHTML = yillikOdemeHtml;
            }
            ...
            // Hedefler ve Artışları
            const hedefArtislariEl = $('hedefArtislari');
            if (hedefArtislariEl && tumYillar.length >= 2) {
              const h2023 = tumYillar.find(y => y.yil === 2023)?.hedefToplam || 0;
              const h2024 = tumYillar.find(y => y.yil === 2024)?.hedefToplam || 0;
              const h2025 = tumYillar.find(y => y.yil === 2025)?.hedefToplam || 0;
              const artis23_24 = h2023 > 0 ? ((h2024 - h2023) / h2023 * 100) : 0;
              ...
              hedefArtislariEl.innerHTML = `
                <table class="tablo">
                  ...
                  <tbody>
                    <tr><td>2023</td><td class="sayi">${fmt(h2023)}</td><td class="sayi">-</td></tr>
                    <tr><td>2024</td><td class="sayi">${fmt(h2024)}</td><td class="sayi ${artis23_24 >= 0 ? 'artis-pozitif' : 'artis-negatif'}">${artis23_24.toFixed(1)}% (${fmt(h2024 - h2023)})</td></tr>
                    <tr><td>2025</td><td class="sayi">${fmt(h2025)}</td><td class="sayi ${artis24_25 >= 0 ? 'artis-pozitif' : 'artis-negatif'}">${artis24_25.toFixed(1)}% (${fmt(h2025 - h2024)})</td></tr>
                  </tbody>
                </table>
              `;
            }

            // Hedef Tutar Artışları
            const hedefTutarArtislariEl = $('hedefTutarArtislari');
            if (hedefTutarArtislariEl && tumYillar.length >= 2) {
              const ht2023 = tumYillar.find(y => y.yil === 2023)?.hedefTutarToplam || 0;
              const ht2024 = tumYillar.find(y => y.yil === 2024)?.hedefTutarToplam || 0;
              const ht2025 = tumYillar.find(y => y.yil === 2025)?.hedefTutarToplam || 0;
              const artis23_24 = ht2023 > 0 ? ((ht2024 - ht2023) / ht2023 * 100) : 0;
              ...
              hedefTutarArtislariEl.innerHTML = `
                <table class="tablo">
                  ...
                  <tbody>
                    <tr><td>2023</td><td class="sayi">${fmt(ht2023)}</td><td class="sayi">-</td></tr>
                    <tr><td>2024</td><td class="sayi">${fmt(ht2024)}</td><td class="sayi ${artis23_24 >= 0 ? 'artis-pozitif' : 'artis-negatif'}">${artis23_24.toFixed(1)}% (${fmt(ht2024 - ht2023)})</td></tr>
                    <tr><td>2025</td><td class="sayi">${fmt(ht2025)}</td><td class="sayi ${artis24_25 >= 0 ? 'artis-pozitif' : 'artis-negatif'}">${artis24_25.toFixed(1)}% (${fmt(ht2025 - ht2024)})</td></tr>
                  </tbody>
                </table>
              `;
            }
            ...
            if (tumYillar.some(y => y.ramazanToplam > 0)) {
              yorumlar.push('<span class="yorum-etiket">Ramazan-ı Şerif Çalışmaları:</span> Ramazan döneminde yürütülen çalışmalar; hem teberru hem de bağ kurma açısından güçlü bir potansiyel taşımaktadır. Mevcut düzey korunarak, yıllar arasında istikrarın sürdürülmesi önemlidir.');
            } else {
              yorumlar.push('<span class="yorum-etiket">Ramazan Verisi Zayıf:</span> Ramazan-ı Şerif çalışmaları ile ilgili kayıtlar sınırlıdır. Bu alandaki fırsatların daha sistematik hedefleme ile güçlendirilmesi önerilir.');
            }
